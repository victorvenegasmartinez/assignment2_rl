...
class DQN(QN):
    ...
    def get_q_values(self, state: torch.Tensor, network: str) -> torch.Tensor:
        """
        Input:
            state: A tensor of shape (batch_size, img height, img width, nchannels x config["hyper_params"]["state_history"])
    ...
    def build(self):
        ...
        if "load_path" in self.config["model_training"].keys():
            print('Loading parameters from file:', self.config["model_training"]["load_path"])
    ...
    def save(self):
        ...
        Saves session
        """
        # if not os.path.exists(self.config["output"]["model_output"]):
        #     os.makedirs(self.config["output"]["model_output"])
        torch.save(self.q_network.state_dict(), self.config["output"]["model_output"])
        # self.saver.save(self.sess, self.config["output"]["model_output"])
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        total_norm = torch.nn.utils.clip_grad_norm_(self.q_network.parameters(), self.config["model_training"]["clip_val"])
